				...
				<tbody id="tbody">
					<c:forEach items="${labs}" var="p" varStatus="status">
						<tr>
							<td>${p.student_id}</td>
							<td>${p.user_name}</td>
							<td><input type="text" style="width:60%;height:20%;" name="${p.student_id}score" value="${p.score}"></td>
							<td><input type="text" style="width:60%;height:20%;" name="${p.student_id}explan" value="${p.explan}"></td>
						</tr>
						 </c:forEach>
				</tbody>
				<script type="text/javascript">
